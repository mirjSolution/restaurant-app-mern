const menus = [
  {
    name: 'Breakfast Menu 1',
    image: '/images/breakfast1.jpg',
    description:
      'Aenean in eros molestie, rhoncus velit eu, aliquet quam. Nunc vehicula sagittis dui. Integer ac enim auctor, fermentum dolor id, iaculis orci. Nulla porttitor efficitur mollis. Donec a condimentum neque. Morbi fermentum rhoncus arcu non cursus. Suspendisse a leo in sem tempor blandit. Nunc tempor id eros id viverra. Ut semper massa massa, id scelerisque quam scelerisque eget. Morbi condimentum tellus ipsum, vel placerat libero mattis nec.',
    category: 'Breakfast',
    price: 9.99,
    orderStock: 10,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Breakfast Menu 2',
    image: '/images/breakfast2.jpg',
    description:
      'Donec eu arcu at dui placerat porta. Etiam venenatis efficitur turpis, sed molestie eros. Mauris hendrerit eget nunc ut venenatis. Curabitur id aliquet urna. Proin venenatis dictum rutrum. Nullam commodo purus non ligula eleifend ullamcorper. ',
    category: 'Breakfast',
    price: 5.99,
    orderStock: 10,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Lunch Menu 1',
    image: '/images/lunch1.jpg',
    description:
      'Etiam mollis diam augue, ac mollis mi scelerisque non. Sed venenatis est mollis magna rutrum, lobortis suscipit turpis fringilla. Nulla vehicula nulla a augue lobortis cursus sollicitudin a erat. Etiam aliquet tincidunt nunc, quis feugiat arcu pharetra in. Suspendisse semper dolor lorem, quis ornare nibh condimentum eget. ',
    category: 'Lunch',
    price: 12.99,
    orderStock: 10,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Lunch Menu 2',
    image: '/images/lunch2.jpg',
    description:
      'Fusce a mi eget odio cursus bibendum. Aenean ac aliquet felis. Cras a augue vehicula, interdum ligula eu, vulputate lectus. Fusce a odio vel enim convallis pulvinar. Fusce ut scelerisque turpis, quis gravida augue. Pellentesque sed felis sit amet tellus lacinia fermentum. Etiam ut mauris sagittis, semper dui in, aliquam elit. ',
    category: 'Lunch',
    price: 12.99,
    orderStock: 8,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Dinner Menu 1',
    image: '/images/dinner1.jpg',
    description:
      'Vivamus consequat sit amet ligula id bibendum. Donec sed purus at arcu laoreet dictum et nec odio. Proin venenatis feugiat metus eu euismod. Mauris sagittis fermentum elit, eu venenatis purus venenatis eget. Praesent scelerisque est velit. Phasellus luctus, diam quis convallis fringilla, nulla lacus feugiat enim.',
    category: 'Dinner',
    price: 10.99,
    orderStock: 5,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Dinner Menu 2',
    image: '/images/dinner2.jpg',
    description:
      'Cras volutpat vulputate euismod. Vestibulum viverra malesuada augue, a laoreet tellus finibus non. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras blandit felis in rutrum faucibus. Phasellus tellus mi, ullamcorper id libero a, pharetra pharetra enim.',
    category: 'Dinner',
    orderStock: 6,
    price: 11.99,
    rating: 0,
    numReviews: 0,
  },
  {
    name: 'Breakfast Menu 3',
    image: '/images/breakfast3.jpg',
    description:
      'Aenean in eros molestie, rhoncus velit eu, aliquet quam. Nunc vehicula sagittis dui. Integer ac enim auctor, fermentum dolor id, iaculis orci. Nulla porttitor efficitur mollis. Donec a condimentum neque. Morbi fermentum rhoncus arcu non cursus. Suspendisse a leo in sem tempor blandit. Nunc tempor id eros id viverra. Ut semper massa massa, id scelerisque quam scelerisque eget. Morbi condimentum tellus ipsum, vel placerat libero mattis nec.',
    category: 'Breakfast',
    price: 9.99,
    orderStock: 10,
    rating: 5,
    numReviews: 4,
  },
  {
    name: 'Breakfast Menu 4',
    image: '/images/breakfast4.jpg',
    description:
      'Donec eu arcu at dui placerat porta. Etiam venenatis efficitur turpis, sed molestie eros. Mauris hendrerit eget nunc ut venenatis. Curabitur id aliquet urna. Proin venenatis dictum rutrum. Nullam commodo purus non ligula eleifend ullamcorper. ',
    category: 'Breakfast',
    price: 5.99,
    orderStock: 10,
    rating: 4,
    numReviews: 4,
  },
  {
    name: 'Lunch Menu 3',
    image: '/images/lunch3.jpg',
    description:
      'Etiam mollis diam augue, ac mollis mi scelerisque non. Sed venenatis est mollis magna rutrum, lobortis suscipit turpis fringilla. Nulla vehicula nulla a augue lobortis cursus sollicitudin a erat. Etiam aliquet tincidunt nunc, quis feugiat arcu pharetra in. Suspendisse semper dolor lorem, quis ornare nibh condimentum eget. ',
    category: 'Lunch',
    price: 12.99,
    orderStock: 10,
    rating: 5,
    numReviews: 2,
  },
  {
    name: 'Lunch Menu 4',
    image: '/images/lunch4.jpg',
    description:
      'Fusce a mi eget odio cursus bibendum. Aenean ac aliquet felis. Cras a augue vehicula, interdum ligula eu, vulputate lectus. Fusce a odio vel enim convallis pulvinar. Fusce ut scelerisque turpis, quis gravida augue. Pellentesque sed felis sit amet tellus lacinia fermentum. Etiam ut mauris sagittis, semper dui in, aliquam elit. ',
    category: 'Lunch',
    price: 11.99,
    orderStock: 8,
    rating: 5,
    numReviews: 4,
  },
  {
    name: 'Dinner Menu 3',
    image: '/images/dinner1.jpg',
    description:
      'Vivamus consequat sit amet ligula id bibendum. Donec sed purus at arcu laoreet dictum et nec odio. Proin venenatis feugiat metus eu euismod. Mauris sagittis fermentum elit, eu venenatis purus venenatis eget. Praesent scelerisque est velit. Phasellus luctus, diam quis convallis fringilla, nulla lacus feugiat enim.',
    category: 'Dinner',
    price: 11.99,
    orderStock: 5,
    rating: 2,
    numReviews: 2,
  },
  {
    name: 'Dinner Menu 4',
    image: '/images/dinner4.jpg',
    description:
      'Cras volutpat vulputate euismod. Vestibulum viverra malesuada augue, a laoreet tellus finibus non. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras blandit felis in rutrum faucibus. Phasellus tellus mi, ullamcorper id libero a, pharetra pharetra enim.',
    category: 'Dinner',
    orderStock: 6,
    price: 11.99,
    rating: 0,
    numReviews: 0,
  },
];

export default menus;
